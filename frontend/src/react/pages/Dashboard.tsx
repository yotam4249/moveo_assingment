...
export default function Dashboard() {
  ...
  /** Patch only one section in the current feed (do NOT replace whole feed)
   * If the new first item equals the one we just downvoted, auto-advance to the next.
   */
  function updateSection(
    section: "news" | "prices" | "insight",
    fresh: FeedResponse,
    justDownvotedId?: string
  ) {
    setFeed((prev) => {
      if (!prev) return fresh;
      const prevRanked = prev.ranked ?? { news: [], prices: [], insight: [] };
    ...
    // adjust indices AFTER feed is set, using a microtask
    queueMicrotask(() => {
      if (section === "news") {
        const list = (fresh.ranked?.news ?? []);
        const first = list[0]?.id;
        // if server still returns the same first card after 👎, show the next one
        setNewsIdx(first && justDownvotedId && first === justDownvotedId ? Math.min(1, Math.max(0, list.length - 1)) : 0);
      }
      if (section === "prices") {
        const list = (fresh.ranked?.prices ?? []);
        const first = list[0]?.id;
        setPriceIdx(first && justDownvotedId && first === justDownvotedId ? Math.min(1, Math.max(0, list.length - 1)) : 0);
      }
      if (section === "insight") {
        const list = (fresh.ranked?.insight ?? []);
        const first = list[0]?.id;
        setInsightIdx(first && justDownvotedId && first === justDownvotedId ? Math.min(1, Math.max(0, list.length - 1)) : 0);
      }
    });
  }
  ...
  async function sendFeedbackAndRefreshSection(
    ...
    try {
      ...
      if (resp?.feed) {
        updateSection(section, resp.feed, decision === "down" ? item.id : undefined);
        return true;
      }
    } catch (e) {
      console.warn("submitFeedback failed:", e);
  ...
  async function voteDown(section: "news" | "prices" | "insight", item: ContentItem) {
    ...
    // local fallback if server didn't return feed
    if (section === "news") setNewsIdx((i) => Math.min(i + 1, Math.max(0, newsList.length - 1)));
    if (section === "prices") setPriceIdx((i) => Math.min(i + 1, Math.max(0, priceList.length - 1)));
    if (section === "insight") setInsightIdx((i) => Math.min(i + 1, Math.max(0, insightList.length - 1)));
  }
