...
export default function Dashboard() {
  ...
  // Fetch a meme once on first load if missing
  const memeBootstrapped = useRef(false);
  useEffect(() => {
    if (!loading && feed && !feed.meme && !memeBootstrapped.current) {
      memeBootstrapped.current = true;
      (async () => { try { await fetchAndSwapMeme(); } catch {} })();
    }
  }, [loading, feed]); // eslint-disable-line react-hooks/exhaustive-deps
